	...
	class Assistance{
		...
		function getSecheduleAssistance($horaLocal, $date){
			...
			if ($result->num_rows > 0) {
				$query = "SELECT schedule_register.idS_Register, student.idStudent, student.name, student.lastName, student.stateAcademic FROM `teacher_schedule_" . $_SESSION['id'] . "` INNER JOIN schedule_register ON schedule_register.idS_Register = teacher_schedule_" . $_SESSION['id'] . ".idScheduleInfo INNER JOIN section ON section.idSection = schedule_register.idSection INNER JOIN level ON section.idLevel = level.idLevel INNER JOIN student ON student.idSection = section.idSection  WHERE student.state = 'A' AND student.verified =1 AND schedule_register.startTime BETWEEN schedule_register.startTime AND '$horaLocal' AND schedule_register.endTime BETWEEN '$horaLocal' AND schedule_register.endTime ORDER BY student.lastName ASC";
				$result = $this->connection->connection->query($query);
